...
SOCKET_ADDRESS = os.getenv("SOCKET_ADDRESS", "192.168.2.14")
SOCKET_PORT = 65432
CONNECTION_TIMEOUT = 10  # 5 seconds timeout
...
def generate_function(function_name, address):
    def wrapped_function(self, *args, **kwargs):
        ...
        sock.settimeout(CONNECTION_TIMEOUT)  # Set timeout
        try:
            sock.connect(address)
        except (ConnectionError, socket.timeout) as e:
            error_msg = f"Failed to establish connection to robot control box at {address[0]}:{address[1]}. "
            error_msg += "Please check:\n"
            error_msg += "1. Robot control box is powered on and connected to network\n"
            error_msg += "2. IP address {address[0]} is correct\n"
            error_msg += "3. Port {address[1]} is open and accessible\n"
            error_msg += "4. Network firewall allows the connection\n"
            error_msg += "5. Try resetting the control box from Teach pendant"
...
class Robot:
    def __init__(self):
        ...
        self.logger.info(f"Attempting to connect to robot control box at {SOCKET_ADDRESS}:{SOCKET_PORT}")
    ...
    def notify_diagnostics(self):
        while True:
            try:
                ...
                self.logger.debug(f"Diagnostics monitoring error: {e}")
